...
class Chip8 {
  ...
  static readonly FONTSET_START_ADDRESS = 0x50;
  static readonly FONTSET = [
  ...
  private pc = Chip8.START_ADDRESS;
  private I = 0;
  private opcode = 0;
  private stack = new Array(16);
  private sp: number = 0;
  private keypad = new Uint8Array(16);
  private _halt = false;
  private shouldRedraw = true;

  private graphics: GPUColor[][] = new Array(Chip8.SCREEN_WIDTH).fill(new Array(Chip8.SCREEN_HEIGHT).fill(GPUColor.White));
  
  /**
   * CPU Registers
   * There are 15 registers, each 8-bits in size.
   */
  private V = new Uint8Array(15);
  ...
  constructor(private canvasContext2D: CanvasRenderingContext2D) {
    ...
    // Load fontset
    Chip8.FONTSET
      .forEach((fontAddress: number, index: number) => {
   ...
   */
  async loadProgram(path: string): Promise<void> {
    const response: Response = await fetch(path);

    if (response.ok) {
      let arrayBuffer = new Uint8Array(await response.arrayBuffer());
      arrayBuffer.forEach((data: number, index: number) => {
  ...
  private drawScreen(): void {
    ...
    this.graphics.forEach((row: GPUColor[], x: number) => {
      row.forEach((pixel: GPUColor, y: number) => {
...
const canvas: HTMLCanvasElement = document.querySelector('#video')!;
